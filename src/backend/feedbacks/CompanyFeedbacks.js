...
export async function saveCompanyFeedbackBackend(uid, vehicle='', vehicleName, feedback) {
	...
	let opinion = { email_sender: user.email, vehicle_name: vehicleInfos.name, vehicle_registration_plate: vehicleInfos.registration_plate, feedback };

	if(user.name)	opinion.name_sender = user.name;
