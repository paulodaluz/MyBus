...
export async function getVehicleFunction({ registrationPlate = '' }) {
	...
	return allVehicleFunctions.find(
		(vehicleFunctions) => vehicleFunctions.registration_plate === registrationPlate
	);
}
...
export async function deleteVehicleFromAllDatabases(
	completeUser,
	idToPassengersToRemove,
	vehiclePlateToRemove
) {
	const [allVehicleFunctions, allVehicles, allUsers] = await Promise.all([
		getAllFunctionsVehicles(),
		getAllVehicles(),
		getAllUsers(),
	]);
	...
	const vehicleFunctionsToRemove = allVehicleFunctions.find(
		(vehicleFunctions) => vehicleFunctions.registration_plate === vehiclePlateToRemove
	);

	await deleteVehicleFunctions(vehicleFunctionsToRemove.id);

	// remover o veiculo da tabela de veiculos
	const vehicleToRemove = allVehicles.find(
		(vehicle) => vehicle.registration_plate === vehiclePlateToRemove
	);
...
export async function deleteVehicleFromUser(completeUser, idToPassengersToRemove) {
	...
	allVehicleCodes = completeUser.codes_private_vehicles.filter(
		(vehicleCode) => vehicleCode !== idToPassengersToRemove
	);
